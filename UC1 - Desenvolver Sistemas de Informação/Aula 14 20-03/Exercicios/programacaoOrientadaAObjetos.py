...
3. Crie uma classe ContaBancaria que tenha os atributos privados numero, titular e saldo e os métodos públicos depositar(), sacar() e consultar_saldo(). O método depositar() deve receber como parâmetro a quantia a ser depositada e adicionar ao saldo. O método sacar() deve receber como parâmetro a quantia a ser sacada e subtrair do saldo, se houver saldo suficiente. O método consultar_saldo() deve retornar o valor do saldo. Crie um objeto dessa classe e teste os métodos.

4. Crie uma classe Funcionario que tenha os atributos nome, salario e cargo e o método calcular_bonus(). Crie também as classes Gerente, Vendedor e Estagiario que herdem da classe Funcionario e sobrescrevam o método calcular_bonus() para retornar 20%, 10% e 5% do salário respectivamente. Crie alguns objetos dessas classes e teste o método calcular_bonus() em cada um.
'''
...
class Animal():
    ...
    def emitirsom(self):
        print(f"O {self.__nome} faz {self.som}")
